...
<script>
/* =========================================================
   EDGE / Ghost Comments UI (iframe) — CTA commenti
   - i commenti vivono in iframe (title="comments-frame")
   - Nasconde “Commenta per primo”
   - Testo: “Iscriviti a ONlettering per poter commentare.”
   - Bottone: “Iscriviti alla newsletter” -> #/portal/signup
   - Nasconde “Sei già iscritto? Accedi” dentro comments frame
   - Observer SOLO nel frame, throttled, auto-disconnect
   ========================================================= */
onlOnReady(() => {
  ...
  function findCommentsIframe(){
    return document.querySelector('iframe[title="comments-frame"]')
      || document.querySelector('.gh-comments iframe')
      || null;
  }

  function applyInside(doc){
    if (!doc || !doc.body) return;

    // CSS “semplice” per nascondere testi (fallback: faremo anche match testuale)
    if (!doc.getElementById('onl-comments-fix-style')) {
      const st = doc.createElement('style');
      st.id = 'onl-comments-fix-style';
      st.textContent = `
        /* fallback: se troviamo stringhe esatte le nascondiamo via attribute */
        [data-onl-hide="1"]{ display:none !important; }
      `;
      doc.head && doc.head.appendChild(st);
    }

    // 1) Nascondi “Commenta per primo”
    doc.querySelectorAll('h1,h2,h3,h4,p,div,span').forEach(el => {
      const t = norm(el.textContent);
      if (t === "commenta per primo") {
        el.setAttribute('data-onl-hide','1');
      }
    });

    // 2) Nascondi “Sei già iscritto? Accedi” (o varianti)
    doc.querySelectorAll('p,div,span').forEach(el => {
      const t = norm(el.textContent);
      if (t.includes("sei già iscritto") && t.includes("accedi")) {
        // evita di nascondere container enormi: solo se è “riga” corta
        if ((el.textContent || "").length < 120) el.setAttribute('data-onl-hide','1');
      }
    });

    // 3) Imposta/crea la riga “Iscriviti a ONlettering per poter commentare.”
    // Cerchiamo un nodo che già parla di commentare/iscrizione, e lo sostituiamo.
    let line = null;

    // a) preferisci un paragrafo che contiene “per poter commentare”
    line = Array.from(doc.querySelectorAll('p,div,span'))
      .find(el => norm(el.textContent).includes("per poter commentare"));

    // b) oppure uno che contiene “iscriviti” e “comment”
    if (!line) {
      line = Array.from(doc.querySelectorAll('p,div,span'))
        .find(el => {
          const t = norm(el.textContent);
          return t.includes("iscriviti") && (t.includes("comment") || t.includes("commentare"));
        });
    }
    ...
    // 4) Trasforma CTA “Accedi ora / Accedi” nel bottone newsletter
    // Possibili: button, a, elementi con role=button
    ...
    const targetBtn =
      ...
      btns.find(b => {
        const t = norm(b.textContent);
        return t.includes("accedi");
      });

    if (targetBtn) {
      targetBtn.textContent = WANT_BTN;

      // rendilo “click -> signup”
      if (!targetBtn.__onl_signup_bound) {
        targetBtn.__onl_signup_bound = true;
        targetBtn.addEventListener('click', openSignup, true);
      }

      // se è un link, prova anche a settare href
      if (targetBtn.tagName && targetBtn.tagName.toLowerCase() === 'a') {
        targetBtn.setAttribute('href', SIGNUP_HASH);
      }
    }

    // 5) Se nel frame è stato creato anche un bottone “Iscriviti alla newsletter” duplicato, tienine uno solo
    const dupes = btns.filter(b => b !== targetBtn && norm(b.textContent) === norm(WANT_BTN));
    dupes.forEach(b => b.parentNode && b.parentNode.removeChild(b));
  }

  function attachToIframe(iframe){
    if (!iframe || iframe.__onl_comments_hooked) return;
    iframe.__onl_comments_hooked = true;
    ...
    // burst iniziale (il frame spesso rende dopo qualche tick)
    let n = 0;
    const burst = setInterval(() => {
      n++;
      const ok = tryApply();
      if (ok && n > 6) clearInterval(burst);
      if (n > 60) clearInterval(burst); // ~6s
    }, 100);

    // observer dentro frame, ma “safe” e temporaneo
    const bindObserver = () => {
      try{
        const doc = iframe.contentDocument || iframe.contentWindow?.document;
        if (!doc || !doc.documentElement) return;

        if (doc.__onl_comments_observer_attached) return;
        doc.__onl_comments_observer_attached = true;
        ...
        setTimeout(() => {
          try{ mo.disconnect(); }catch(_){}
          doc.__onl_comments_observer_attached = false;
        }, 12000);
  ...
  // ciclo di aggancio: l’iframe può comparire dopo il caricamento
  let k = 0;
  const hunt = setInterval(() => {
    k++;
    const f = findCommentsIframe();
    if (f) attachToIframe(f);
    if (f || k > 120) clearInterval(hunt); // ~12s max
  }, 100);
